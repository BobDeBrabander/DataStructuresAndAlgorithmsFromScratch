...
class MyGraph {
    val nodes = mutableListOf<MyNode>()
    ...
    fun edgeExists(n1: MyNode, n2: MyNode) = this.edges.any{it.source == n1 && it.target == n2}

    fun removeEdge(edge: MyEdge){
        removeEdge(edge.target, edge.source)
    }

    fun removeEdge(n1: MyNode, n2: MyNode) {
    ...
    class NotAllowedException(msg: String) : IllegalArgumentException(msg)
}